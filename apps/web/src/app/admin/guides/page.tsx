...
export default function GuidesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await adminGuides.update(editingId, formData);
      } else {
        await adminGuides.create(formData);
      }
      setShowForm(false);
      setEditingId(null);
      setFormData({});
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-600 mt-2">Manage educational guide pages</p>
        </div>
        <button
          onClick={() => {
            setShowForm(true);
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8">
          <h2 className="text-xl font-bold mb-4">{editingId ? 'Edit' : 'Create'} Guide</h2>
              ...
              {guides.map((guide) => (
                <tr key={guide.id} className="hover:bg-slate-50">
                  <td className="px-6 py-4">
                    ...
                    <div className="text-sm text-slate-500">{guide.slug}</div>
                  </td>
