...
import { getMakesForCountry, getModelsForMake } from '../vehicleData';
...
export default function StepVehicle({ form, locale }: StepVehicleProps) {
  ...
  const makes = getMakesForCountry(country);

  const customMakeLabel = country === 'BR' ? 'Outra marca...' : 'Other make...';
  const customModelLabel = country === 'BR' ? 'Outro modelo...' : 'Other model...';
          ...
          <div key={idx} className="bg-white rounded-xl border border-slate-200 p-5 space-y-5">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-4">
              ...
              <FormAutocomplete
                ...
                value={vehicle.make}
                onChange={(val) => {
                  setValue(`vehicles.${idx}.make`, val, { shouldValidate: true });
                  if (val !== vehicle.make) {
                    setValue(`vehicles.${idx}.model`, '');
                  }
                }}
                error={errors.vehicles?.[idx]?.make?.message}
                placeholder={country === 'BR' ? 'Buscar marca...' : 'Search make...'}
                required
                allowCustom
                customLabel={customMakeLabel}
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-4">
              <FormAutocomplete
                ...
                options={models}
                value={vehicle.model}
                onChange={(val) => setValue(`vehicles.${idx}.model`, val, { shouldValidate: true })}
                error={errors.vehicles?.[idx]?.model?.message}
                placeholder={country === 'BR' ? 'Buscar modelo...' : 'Search model...'}
                required
                allowCustom={true}
                customLabel={customModelLabel}
